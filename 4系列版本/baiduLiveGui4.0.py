...
class BaiduLive(QMainWindow):
    ...
    def __init__(self):
        ...
        #绑定 onrTRrainSendButton
        self.ui.onetrainSendButton.clicked.connect(self.oneTrainSendButton_click)
        #绑定 oneTrainStopBNutton
        self.ui.onetrainStopButton.clicked.connect(self.oneTrainStopButton_click)
        try:
            self.get_version()
    ...
    #但账号轮训发送
    def oneTrainSendButton_click(self):
        # 检测激活码有效性
        ...
        if CDKEY == 'OK':
            ...
            if ckName != 'null':
                single_parth = 'Single.txt'
                cycles_num = self.ui.oneCycles.text()
                oneTrainFirst = self.ui.oneTrainFirst.text()
                oneTrainLast = self.ui.oneTrainLast.text()

                self.ui.onetrainSendButton.setEnabled(False)
                self.ui.onetrainSendButton.setText('正在发送中...')
                self.ui.onetrainStopButton.setEnabled(True)

                with open('.ini/oneTrainSend.ini','a',encoding='utf-8') as f:
                    f.write(
                        '\n'+single_parth+
                        '\n'+cycles_num+
                        '\n'+oneTrainFirst+
                    ...
                    f.close()
                self.printf('=' * 25)
                self.printf('正在启动单账号评论循环功能！')
                self.printf('=' * 25)
                onetrainSend = 'oneCkTrainSend.exe'
                os.startfile(onetrainSend)
            else:
    ...
    #单账号轮训停止发送
    def oneTrainStopButton_click(self):
        try:
            # os.remove('oneTrainSend.ini')
            os.system('taskkill /f /im oneCkTrainSend.exe')
        except:
            pass
        self.ui.onetrainStopButton.setEnabled(False)
        self.ui.onetrainSendButton.setEnabled(True)
        self.ui.onetrainSendButton.setText('发送')
        self.printf('=' * 25)
        self.printf('单账号弹幕评论停止>>>>>>>>>>>>>')
        self.printf('=' * 25)
    ...
    def instant_stop(self):
        try:
            os.remove('.ini/instantComments.ini')
            os.system('taskkill /f /im InstantComments.exe')
        except:
            pass
    ...
    def get_stopButton_click(self):
        try:
            ...
            os.system('taskkill /f /im sendComments.exe')
        except:
            pass
    ...
    def get_stopCaidan_click(self):
        try:
            ...
            os.system('taskkill /f /im SendEggshell.exe')
        except:
            pass
