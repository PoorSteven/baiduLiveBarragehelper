...
class BaiduLive(QMainWindow):
    ...
    def closeEvent(self, event):
        if self.ui.sendButton.isEnabled()!=True:
            QMessageBox.question(self,'提示','关闭软件之前，请先关闭弹幕发送！')
            event.ignore()
        elif self.ui.instantSendButon.isEnabled()!=True:
            QMessageBox.question(self, '提示', '关闭软件之前，请先关闭即时弹幕发送！')
            event.ignore()
        elif self.ui.sendCaidan.isEnabled()!=True:
            QMessageBox.question(self, '提示', '关闭软件之前，请先关闭彩蛋弹幕发送！')
            event.ignore()
        else:
            QMessageBox.question(self, '提示', '感谢老板使用，福生无量天尊！')
            event.accept()
    ...
    def get_version(self):
        ...
        if online_vsersion != local_vserion:
            ...
            QMessageBox.information(self,'版本更新','软件有新版本了！可以下载新版本哦！')
        QApplication.processEvents()
    ...
    def ck_import(self):
        try:
            ...
            with open(filepath,'r',encoding='utf-8')as f:
                ...
                QMessageBox.question(self, '爱你的提示', '老板，重新导入CK后，得点击一下保存配置的按钮哦！')
    ...
    def ads_import(self):
        try:
            ...
            with open(filepath,'r',encoding='utf-8')as f:
                ...
                QMessageBox.question(self, '爱你的提示', '老板，重新导入ADS后，得点击一下保存配置的按钮哦！')
        except:
            # 提示ADS数量
    ...
    def single_import(self):
        try:
            ...
            with open(filepath,'r',encoding='utf-8')as f:
                ...
                QMessageBox.question(self, '爱你的提示', '老板，重新导入single后，得点击一下保存配置的按钮哦！')
        except:
            # 提示ADS数量
    ...
    def online_popul_click(self):
        ...
        if os.path.isfile('.ini/onlineTime.ini') == True:
            os.remove('.ini/onlineTime.ini')
        # 检测激活码有效性
        CDKEY = self.Detection_cdkey()
        print(CDKEY)
        ...
        try:
            if CDKEY == 'OK':
                onlineTime = self.ui.onlineTime.text()
                room_id = self.get_room_id()
                # print(onlineTime)
                with open('.ini/onlineTime.ini','w',encoding='utf-8')as o:
                    o.write(onlineTime+'\n'+room_id)
    ...
    def get_mac_address(self):
        ...
        for physical_disk in c.Win32_DiskDrive():
            ...
            disk = physical_disk.SerialNumber
            if disk == None:
                disk = '2621228281'
            disk_lists.append(disk.strip())
        disk_num = ''.join(disk_lists)
        md5 = hashlib.md5(disk_num.encode('utf-8')).hexdigest()
    ...
    #
    #     with open('.ini/mac.ini', 'r', encoding='utf-8')as f:
    ...
    def Detection_cdkey(self):
        ...
        if cdkey_type == '天卡':
            ...
            sql_fail_time = self.sql_fail_time(90)
        ...
        if len(local_cdkey) == 55:
            ...
            if result[0] == 1 and result[1] == None:
                ...
                try:
                    ...
                    con.rollback()
            # 程序激活码Mac地址正确时，运行程序
            elif result[0] == 1 and result[1] == self.get_mac_address() and self.time_stamp(self.get_now_time()) < self.time_stamp(result[2]):
